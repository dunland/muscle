...
int num_of_swell_taps_quantized[numInputs]; // will be used in both swell_rec() and swell_beat(). serves as cc_val for MIDI notes.
int swell_stroke_interval[numInputs];       // will be needed for timed replay
int swell_state_quantized[numInputs];
...
void swell_init_quantized() // initialize arrays for swell functions (run in setup())
{
    for (int i = 0; i < numInputs; i++)
    {
        swell_state_quantized[i] = 1; // waits for first tap
        num_of_swell_taps_quantized[i] = 0;
        cc_val[i] = 10;
...
void swell_tap_quantized(int instr) // remembers beat stroke position
{
    ...
    if (swell_state_quantized[instr] == 1) // first hit
    {
        //    if (millis() > lastSwellRec + 6000) // reset if tap was too long
        //    {
        //      swell_state_quantized[instr] = 1; // waits for first tap
        ...
        interrupts();
        swell_state_quantized[instr] = 2;

        lastSwellRec = millis();
    }

    else if (swell_state_quantized[instr] == 2) // second hit
    {
        num_of_swell_taps_quantized[instr]++; // will define the cc_val for replayed MIDI notes

        cc_val[instr] += 15;                                         // ATTENTION: must rise faster than it decreases! otherwise swell resets right away.
        ...
        if ((current_swell_beatPos - previous_swell_beatPos[instr]) <= 32) // only do this if interval was not too long
        {
            ...
            float a = float(swell_beatPos_sum[instr]) / float(num_of_swell_taps_quantized[instr]); //rounds up or down
            a += 0.5;
...
void swell_play_quantized(int instr) // updates once a 32nd-beat-step
{
    if (swell_state_quantized[instr] == 2)
    {
        ...
        if (swell_beatStep[instr] == 0) // on swell beat
        {
            ...
            output_string[instr] += "] ";
            output_string[instr] += num_of_swell_taps_quantized[instr];
            output_string[instr] += "/";
            output_string[instr] += swell_beatPos_sum[instr];
            output_string[instr] += " (";
            output_string[instr] += cc_val[instr];
            output_string[instr] += ") ";
            output_string[instr] += "\t";
        ...
        (3. reset that parameter, if you like) */
            ...
            {
                swell_state_quantized[instr] = 1; // waits for first tap
                num_of_swell_taps_quantized[instr] = 0;
                cc_val[instr] = 10;
                swell_beatPos_sum[instr] = 0;
                swell_beatStep[instr] = 0;
            } //TODO: reset!
...
//////////////////////// SWELL TAP UNQUANTIZED ////////////////////////
///////////////////////////////////////////////////////////////////////
int swell_state[numInputs];
unsigned long first_swell_tap[numInputs];
int num_of_swell_taps[numInputs];
int swell_sum[numInputs];
int swell_effect[numInputs];
unsigned long last_swell_decrease[numInputs];
...
void swell_init()
{
    for (int i = 0; i < numInputs; i++)
    {
        ...
        swell_interval[i] = 0;
        swell_timer[i] = 0;
        flag_play_swell_MIDI[i] = false;
        swell_effect[i] = 0;
        last_swell_decrease[i] = 0;
    }
}

void swell_tap(int in01)
{
    static boolean swell_started[numInputs];

    switch (swell_state[in01])
    {

    case 1:                                          // first hit
        if (millis() > first_swell_tap[in01] + 4000) // reinitiate swell_tap if not used for four seconds
        {
            num_of_swell_taps[in01] = 0;
            swell_sum[in01] = 0;
            swell_effect[in01] = 0;
            swell_interval[in01] = 0;
        }
        if (swell_effect <= 0 && swell_started[in01] == true)
        {
            num_of_swell_taps[in01] = 0;
            swell_sum[in01] = 0;
            swell_effect[in01] = 0;
        ...
        first_swell_tap[in01] = millis(); // record time of first hit
        swell_state[in01] = 2;            // next: wait for second hit

        break;

    case 2: // second hit

        if (millis() < first_swell_tap[in01] + 2000) // only record tap if interval was not too long
        {

            num_of_swell_taps[in01]++;
            swell_effect[in01] += 10;
            swell_effect[in01] = (swell_effect[in01] > 127) ? 127 : swell_effect[in01]; // max cc_val = 127
            swell_sum[in01] += millis() - first_swell_tap[in01];
            swell_interval[in01] = swell_sum[in01] / num_of_swell_taps[in01];

            swell_state[in01] = 1;
        }

        if (first_swell_tap[in01] > 2000) // forget tap if time was too long
        {
            swell_state[in01] = 1;
...
void swell_play(int in00)
{
    static boolean flag_swell_copy[numInputs];
    noInterrupts();
    flag_swell_copy[in00] = flag_play_swell_MIDI[in00];
    interrupts();

    if (flag_swell_copy[in00] == true)
    {
        if (swell_effect[in00] > 0)
        {
            output_string[in00] = String(swell_interval[in00]);
            output_string[in00] += " (";
            output_string[in00] += String(swell_effect[in00]);
            output_string[in00] += ") ";

            MIDI.sendControlChange(cc_chan[in00], swell_effect[in00], 2);
            MIDI.sendNoteOn(notes_list[in00], 127, 2);

            noInterrupts();
            flag_play_swell_MIDI[in00] = false;
            interrupts();
        }
    }

    if (millis() > last_swell_decrease[in00] + swell_interval[in00]) // probably a bit strange in the beginning, since swell_interval = 0
    {
        //MIDI.sendNoteOff(notes_list[in00], 127, 2);
        if (swell_effect[in00] > 0)
            swell_effect[in00]--;
        last_swell_decrease[in00] = millis();
    }
}
