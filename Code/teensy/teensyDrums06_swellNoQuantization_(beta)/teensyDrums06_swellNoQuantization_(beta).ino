...
int pinAction[] = {1, 1, 1, 6, 6, 6, 4, 1}; // array to be changed within code loop.
...
void setup()
{
    ...
    swell_init_quantized(); // prepares pinAction 5
    swell_init();           // prepares pinAction 6
...
/* -------------------------- TIMED INTERRUPTS ------------------------- */
/* --------------------------------------------------------------------- */
volatile int swell_timer[numInputs];
volatile boolean flag_play_swell_MIDI[numInputs];
volatile int swell_interval[numInputs];
...
void samplePins()
{
    ...
    for (int pinNum = 0; pinNum < numInputs; pinNum++)
    {
        ...
        //swell_timer[pinNum] = swell_timer[pinNum]++;
        if (pinAction[pinNum] == 6)
        {
          swell_timer[pinNum] = (swell_timer[pinNum] + 1) % swell_interval[pinNum];
          if (swell_timer[pinNum] == 0) flag_play_swell_MIDI[pinNum] = true;
        }
    }
}
// -----------------------------------------------------------------------------
...
void loop()
{
    ...
    for (int i = 0; i < numInputs; i++)
    {
        if (stroke_detected(i)) // evaluates pins for activity repeatedly
        {
            ...
            case 5: // "swell", not working too well
                setInstrumentPrintString(i, pinAction[i]);
                swell_tap_quantized(i);
                break;
  ...
  */
    ...
    if (current_beat_pos != last_beat_pos)
    {
        ...
        // -------------------------- PIN ACTIONS: ------------------------
        // updating once a 32nd-beat-step
        for (int i = 0; i < numInputs; i++)
        {
            if (pinAction[i] == 5)
                swell_play_quantized(i);

            if (pinAction[i] == 6)
                swell_play(i); // turns MIDI notes on/off if flag was set in timer.
