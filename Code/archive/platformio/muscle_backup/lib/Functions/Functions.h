...
void calculateNoiseFloor()
{
  ...
  for (int pinNum = 0; pinNum < Globals::numInputs; pinNum++)
  {
    ...
    {
      while (analogRead(Globals::pins[pinNum]) < 700 + Calibration::calibration[pinNum][0])
    ...
    for (int n = 0; n < 400; n++)
    {
      ...
      totalSamples += analogRead(Globals::pins[pinNum]);
    }
    Calibration::noiseFloor[pinNum] = totalSamples / 400;
    digitalWrite(Globals::leds[pinNum], LOW);
    led_idx++;
    Serial.print("noiseFloor = ");
    Serial.println(Calibration::noiseFloor[pinNum]);
  }
...
void checkFootSwitch()
{
  ...
  if (switch_state != last_switch_state && millis() > last_switch_toggle + 20)
  {
    if (switch_state == LOW)
    {
      Hardware::footswitch_pressed();
      Serial.println("Footswitch pressed.");
    }
    else
    {
      Hardware::footswitch_released();
...
// int swell_val[Globals::numInputs]; // this should be done in the swell section, but is needed in print section already... :/
...
void setInstrumentPrintString(int instr, int incoming_pinAction)
{
  ...
  case 1: // monitor: just print what is being played
    if (instr == Kick)
      Debug::output_string[instr] = "■\t"; // Kickdrum
    else if (instr == Cowbell)
      Debug::output_string[instr] = "▲\t"; // Crash
    else if (instr == Standtom1)
      Debug::output_string[instr] = "□\t"; // Standtom
    else if (instr == Standtom2)
      Debug::output_string[instr] = "O\t"; // Standtom
    else if (instr == Hihat)
      Debug::output_string[instr] = "x\t"; // Hi-Hat
    else if (instr == Tom1)
      Debug::output_string[instr] = "°\t"; // Tom 1
    else if (instr == Snare)
      Debug::output_string[instr] = "※\t"; // Snaredrum
    else if (instr == Tom2)
      Debug::output_string[instr] = "o\t"; // Tom 2
    else if (instr == Ride && Globals::numInputs>7)
      Debug::output_string[instr] = "!xx\t"; // Ride
    else if (instr == Crash1 && Globals::numInputs>7)
      Debug::output_string[instr] = "!-X-\t"; // Crash
    else if (instr == Crash2 && Globals::numInputs>7)
      Debug::output_string[instr] = "!-XX-\t"; // Crash
    break;
  ...
  case 3: // add an ! if pinAction == 3 (replay logged rhythm)
    ...
    //   Debug::output_string[instr] = swell_val[instr];
...
int num_of_swell_taps[Globals::numInputs];     // will be used in both swell_rec() and swell_beat(). serves as swell_val for MIDI notes.
...
void getTapTempo()
{
  ...
  case 2: // second hit
    ...
    if (millis() < timeSinceFirstTap + 2000) // only record tap if interval was not too long
    {
      ...
      Timing::tapInterval = clock_sum / num_of_taps;
      Serial.print("new tap Tempo is ");
      Serial.print(60000 / Timing::tapInterval);
      Serial.print(" bpm (");
      Serial.print(Timing::tapInterval);
      Serial.println(" ms interval)");

      Timing::current_BPM = 60000 / Timing::tapInterval;
      tapState = 1;
...
{
  if (Debug::do_send_to_processing)
...
void smoothen_dataArray(struct Score::TOPOGRAPHY_16 (*struct_ptr), int instr)
{
  int entries = 0;
  int squared_sum = 0;
  int regular_sum = 0;
