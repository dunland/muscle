...
const int threshold[] = {20, 120, 150, 60}; // hihat, crash1, ride, Standtom
/*
...
const int rhythm_dividend[] = {1, 3, 2, 1, 3, 1, 1, 3, 1, 1, 3, 1, 1, 1, 1};
const int rhythm_divisor[] = {1, 4, 3, 2, 8, 3, 4, 16, 6, 8, 32, 12, 16, 24, 32};
...
void loop() {
  ...
  printf("bpm: %d\tint: %d\tNotes: %d, %d\tRtm: (%d/%d), (%d/%d)\ttap: %d\n", bpm, clock_interval, notes_list[note_idx[1]], notes_list[note_idx[2]], rhythm_dividend[rhythm_idx[1]], rhythm_divisor[rhythm_idx[1]], rhythm_dividend[rhythm_idx[2]], rhythm_divisor[rhythm_idx[2]], tapState);
  ...
  if (pinValue(pins[1]) > threshold[1] && millis() >= (lastHit_Pin[1] + 1000))
  {
    ...
    {
      rhythm_idx[i] = (rhythm_idx[i] + 1) % 15;
    }
  }
  ...
  if (bpm != 0)
  {
    if (timeSinceLastPulse >= (clock_interval / 24))
    {
      // -------------------------------------- MIDI Clock Sync
      ...
      */
      ...
      if ((pulseCount * rhythm_dividend[rhythm_idx[1]] / rhythm_divisor[1]) % 24 == 0)
      {
      ...
      if ((pulseCount * rhythm_dividend[rhythm_idx[2]] / rhythm_divisor[2]) % 24 == 0)
      {
