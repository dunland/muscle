...
void run_A_25()
{
    ...
    case 2: // change CC only
        if (Globals::active_song->get_setup_state())
        {
            ...
            Drumset::tom2->shuffle_cc(Drumset::kick->midiTargets.back(), true);     // set a random midi CC channel
            Drumset::standtom->shuffle_cc(Drumset::kick->midiTargets.back(), true); // set a random midi CC channel
