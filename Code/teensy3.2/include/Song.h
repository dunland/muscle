...
public:
    ...
    Song(Callback f, String songName) : trigger_function(f)
    {
        ...
        notes.list.push_back(int(random(36, 48)));
        Devtools::print_to_console("active_score->note[0] = ");
        Devtools::println_to_console(notes.list[0]);
        name = songName;
        setup_state = true;
    }
    ...
    struct notes
    {
        ...
        int getNote(int index) // returns a note, even if list is empty
        {
            int note;
            if (list.size() > 1)
                note = list.at(index % sizeof(list));
            else
            {
                note = 0;
                Devtools::println_to_console("no notes in song! returning 0");
            }
            return note;
        }
